#!/usr/bin/env bash
# 發布 AI Global Explorer 到 VS Code Marketplace（複製自 markdown-ext/scripts/publish.sh 後調整）。
#
...
// Unreleased 的內容整段搬到新版本號下，標題留空給下一輪；版本標題沿用本專案的「## x.y.z — 日期」。
...
if command -v code >/dev/null 2>&1; then
  ...
  printf '    已安裝 %s 到預設 profile，請在 Activity Bar 開啟 AI Global Explorer 確認後再繼續。\n' "$vsix"
else
...
echo "==> 發布"
npx --no-install vsce publish --packagePath "$vsix" --allow-missing-repository
